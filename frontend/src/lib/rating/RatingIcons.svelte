...
<div class="flex flex-row items-center">
    {#key rating}
        <div class={twMerge(divClass, $$props.class)}>
            {#if count}
                {#if edit}
                    <svelte:component class="cursor-pointer" on:click this={icon} fillPercent={100} {size}/>
                {:else }
                ...
                <slot/>
            {:else}
                {#each Array(fullStars) as star, index}
                    {#if edit}
                        <svelte:component class="cursor-pointer" on:click={() => rating = index + 1} this={icon} {size} fillPercent={100}
                                          id={fullStarId}/>
                    {:else}
                ...
                {#if percentRating}
                    {#if edit}
                        <svelte:component class="cursor-pointer" on:click={() => rating = fullStars + 1 } this={icon} {size}
                                          fillPercent={percentRating} id={partialId}/>
                    {:else}
                        <svelte:component this={icon} {size} fillPercent={percentRating} id={partialId}/>
                    {/if}
                {/if}
                {#each Array(grayStars) as star, index}
                    {#if edit}
                        <svelte:component class="cursor-pointer" on:click={() => rating = total - grayStars + 1 + index} this={icon} {size}
                                          fillPercent={0} id={grayStarId}/>
                    {:else}
