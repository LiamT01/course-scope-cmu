<script lang="ts">
	...
	const onSubmit: (e: Event) => void = async (e) => {
		const response = await submitRatingWithinPage(
			e,
			{
				rating_id: ratingID,
	...
	function syncScroll() {
		...
		if (comment && preview) {
			preview.scrollTop =
				(comment.scrollTop * (preview.scrollHeight - preview.clientHeight)) /
...
<form {...$$restProps} class={twMerge('flex flex-col gap-y-4', $$props.class)} on:submit={onSubmit}>
	<div class="space-y-4">
		...
		<div class="space-y-4 text-sm font-medium">
			...
			<div
				class="grid grid-rows-6 gap-y-4 sm:grid-flow-col sm:grid-rows-3 md:grid-flow-row md:grid-cols-3 md:grid-rows-none"
			>
				<div class="grid grid-cols-[76px_max-content] items-center">
					<span>Teaching:</span>
		...
		<div class="flex flex-col gap-y-4">
			<div class="space-y-4">
				...
				<div class="grid {markdownPreview ? 'md:grid-cols-2' : ''} gap-4">
					...
					/>
					{#if markdownPreview}
						<div id="preview" class="max-h-[420px] overflow-auto rounded-lg border p-4">
