...
{#if stats}
	<div class={twMerge('flex flex-col gap-y-4', $$props.class)}>
		<div class="flex w-full flex-col items-center gap-y-2">
			<p class="ml-10 flex gap-x-2">
				<span class="text-5xl font-medium">{roundFixed(stats.avg_overall, 1)}</span>
				<RatingStats
					key="course-overall-rating-{key}"
					rubricName="overall rating"
					stats={mapToPercentage(stats.overall)}
				/>
			</p>
			<div class="ml-6 flex items-center justify-center gap-x-2">
			...
			<div class="flex w-full flex-col items-center justify-around sm:flex-row lg:flex-col">
				<div
					class="grid grid-cols-[72px_minmax(0,1fr)_max-content] items-center gap-x-4 text-sm font-medium"
				>
					<span>Teaching:</span>
					...
					<RatingStats
						key="course-teaching-quality-{key}"
						rubricName="teaching quality"
					...
					<RatingStats
						key="course-materials-quality-{key}"
						rubricName="materials quality"
					...
					<RatingStats
						key="course-amount-value-{key}"
						rubricName="amount of value"
				...
				>
					...
					<RatingStats
						key="course-difficulty-{key}"
						rubricName="difficulty"
		...
		<div class="flex flex-col gap-y-4">
			...
			<!--                Taking / Took it (0)-->
			<!--            </OutlineButton>-->
			<PrimaryButton on:click={onContributeRating} size="sm">Rate it!</PrimaryButton>
		</div>
	</div>
{/if}
