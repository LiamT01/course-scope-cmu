<script lang="ts">
	...
	import { blur, fade, fly, slide, type SlideParams } from 'svelte/transition';
	...
	function multiple(node: HTMLElement, params: SlideParams | undefined) {
	...
	function clickOutsideWrapper(node: HTMLElement, callback: () => void) {
		return activateClickOutside
			? clickOutside(node, { callback, excludedElement: null })
			: undefined;
	}
</script>
