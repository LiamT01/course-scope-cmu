<script lang="ts">
    ...
    export let activeClass: string = 'text-gray-600 bg-gray-200 dark:border-gray-700 dark:bg-gray-700 dark:text-white';
...
<nav aria-label={ariaLabel} class={$$props.class}>
    <ul class={twMerge(ulClass, table && 'divide-x rtl:divide-x-reverse dark divide-gray-700 dark:divide-gray-700', $$props.class)}>
        <li>
            <PaginationItem class={table ? 'rounded-l' : 'rounded-s-lg'} {large} {normalClass} on:click={previous}>
        ...
        {/each}
        <li>
            <PaginationItem class={table ? 'rounded-r' : 'rounded-e-lg'} {large} {normalClass} on:click={next}>
