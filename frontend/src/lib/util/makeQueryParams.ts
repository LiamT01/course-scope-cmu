...
export default function makeQueryParams(params: RatingParams) {
	...
	const semesterQuery =
		params.semester && params.semester !== 'all' ? `semester=${params.semester}` : null;
	...
	const instructorIDsQuery =
		params.instructorIDs && params.instructorIDs.length > 0
			? params.instructorIDs.map((id) => `instructor_ids=${id}`).join('&')
			: null;
	const overallQuery =
		params.overall && params.overall !== 'all' ? `overall=${params.overall}` : null;
