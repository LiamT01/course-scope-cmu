...
<div class="py-4 sm:py-8 border-y space-y-2 sm:space-y-4" id="rating-{rating.id}">
    <div class="flex flex-col gap-y-2 justify-between items-start">
        ...
        <div class="flex flex-wrap gap-x-4 items-center">
            ...
            <div class="flex gap-x-4 flex-wrap gap-y-2">
                {#if courseOffering}
                    <div class="ml-2 text-xs flex items-center gap-x-2">
                        <span>{courseOffering.semester} {courseOffering.year}</span>
                    </div>
                {/if}
                <CourseInfo class={!showCourseDetail ? "sm:hidden" : ""} {course} {courseOffering}
    ...
    <div>
        <div class="grid max-w-[640px] grid-flow-col grid-rows-6 gap-y-2 text-xs font-medium text-gray-600 dark:text-gray-400 sm:grid-flow-row sm:grid-cols-3 sm:grid-rows-none sm:gap-x-8">
    ...
    <div class="flex flex-col justify-between gap-y-2 sm:flex-row">
        <div class="w-full order-first flex items-center justify-between gap-x-8 sm:order-none">
            <div class="flex gap-x-8 items-center">
                <LikesDislikes dislikedByUser={rating.disliked_by_viewer}
